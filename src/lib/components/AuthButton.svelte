...
{#if !firebaseEnabled}
	<!-- Auth not configured: hide controls -->
...
{:else if $auth.user}
	<div class="relative">
		...
		>
			...
			{:else}
				<span
					class="grid size-7 place-items-center rounded-full bg-primary text-xs text-on-primary"
				>
...
{:else}
	<div class="relative flex items-center gap-1.5">
		<button
			class="grid size-9 place-items-center rounded-full border border-border bg-surface transition-colors hover:bg-muted"
		...
		>
			<svg class="size-4 fill-foreground" viewBox="0 0 24 24" aria-hidden="true">
				<path
		...
		{#if error}
			<span
				class="absolute right-0 top-11 w-56 rounded-md border border-destructive/40 bg-surface px-3 py-1.5 text-xs text-destructive shadow-lg"
				role="alert"
			>
				{error}
			</span>

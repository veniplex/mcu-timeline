...
<div class="relative mt-8 pb-4">
  ...
  <div
    class="absolute bottom-4 left-1/2 w-0.5 -translate-x-1/2 {grouped
      ? 'top-30'
      : 'top-4'}"
    style={grouped
  ...
  {#each bands as band, bandIdx (band.phase + "-" + band.items[0]?.key)}
    ...
    <section
      id={grouped ? `phase-${band.phase}` : undefined}
      class={grouped ? "scroll-mt-24 first:pt-0 " : ""}
    >
      {#if grouped}
      ...
      <ol class="space-y-8 lg:space-y-12 {grouped ? 'pt-14 lg:pt-20' : 'pt-2'}">
        {#each band.items as item, i (item.key)}
          ...
          >
            <span
              class="absolute left-1/2 top-0 z-10 size-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 md:top-1/2 lg:size-4"
              style="border-color: {color}; background-color: {isItemWatched(
                item,
              )
                ? color
  ...
  <div class="relative flex justify-center pt-14 lg:pt-20">
    ...
    >
      <span
        ...
        style="color: color-mix(in srgb, {PHASE_COLORS[6]} 70%, var(--color-muted-foreground))"
